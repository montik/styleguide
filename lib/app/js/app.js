...
const a = angular.module('sgApp', deps)
  ...
  .config(function($compileProvider, $stateProvider, $urlRouterProvider, $locationProvider, localStorageServiceProvider, $ocLazyLoadProvider) {
  ...
  .filter('addSectionWrapper', function() {
    return function(code, wrapper) {
      console.log('wrapping ' + wrapper);
      if (wrapper) {
        console.log('wrapping crazy');
