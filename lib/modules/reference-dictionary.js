  ...
  var prevSections = [],
	...
	function compareFn(first, second) {
		if (!first.reference || !second.reference) {
			console.log("reference-dictionary.js: reference missing");
			return -1;
		}
		return first.reference.length < second.reference.length ? -1 : 1;
	}

  sections.sort(compareFn).forEach(function(section) {
