...
const buildReferenceDictionary = require('./modules/reference-dictionary').build;
...
function generatePugMarkup(section, options) {
	// use styleguide directive sg-pug-options to explicitly mark pug template
	if (!section.markup || !section.sgPugOptions) {
		return;
	}

	const markup = section.markup;

	section.pugOptions = Object.assign(section.pugOptions, {basedir: options.pugBasedir});
	section.pugFunction = pug.compile(markup, section.pugOptions);
	section.markup = section.pugFunction(section.pugOptions);

	section.modifiers.forEach(modifier => {
		const modifierOptions = Object.assign({modifier: modifier.className},
			section.pugOptions);
			modifier.markup = section.pugFunction(modifierOptions);
		});
	}
	...
	function replaceSectionReferences(json, index) {
		json.sections = sectionReferences.replace(json.sections, index);
	}
	...
	function traverseTree(node, parent, functions) {
		functions.forEach(f => f(node, parent));
		node.sections && node.sections.forEach(
			child => traverseTree(child, node, functions));
		}

		// Section -> Section -> ()
		function addWrapper(node, parent) {
			const parentWrapper = parent && parent.sgWrapper;
			if (parentWrapper) {
				const trimmedParentWrapper = parentWrapper.trim();
				if (node.sgWrapper) {
					const trimmedSectionWrapper = node.sgWrapper.trim();
					node.sgWrapper = trimmedParentWrapper.replace(/<sg\-wrapper\-content\/>/, trimmedSectionWrapper);
				} else {
					node.sgWrapper = trimmedParentWrapper;
				}
			}
		}

		// Section -> Section -> ()
		function addPugOptions (node, parent){
			if (node.sgPugOptions) {
				try {
					node.pugOptions = JSON.parse(node.sgPugOptions) || {};
				} catch (error) {
					console.log(`styleguide.js:addPugOptions --  ${node.reference}: ${error}`);
					node.pugOptions = {};
				}
			}

			if (parent && parent.pugOptions) {
				node.pugOptions = Object.assign({}, parent.pugOptions, node.pugOptions);
			}
		}
		...
		module.exports.generate = function(options) {
			try {
				...
				console.log('ERROR: Unable to exclude default styles.' + err);
			}
			...
			return through(throughOpts, bufferFileContents, function(callback) {
				...
				Q.all([parseKSSPromise, parseVariablesPromise]).spread(function(sections, variables) {
					...
					const index = buildReferenceDictionary(styleguide.sections);
					_.forIn(index, (section => {
						if(section.parentReference == undefined && section.sections != undefined) {
							traverseTree(section, null, [addWrapper, addPugOptions]);
						}
					}));

					if (options.enablePug) {
						_.each(styleguide.sections, _ => generatePugMarkup(_, options));
					}

					_.each(styleguideProcessors, function(processor) {
						processor(styleguide, index);
					});
