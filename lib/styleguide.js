...
function generatePugMarkup(json, options) {
  ...
  */
  ...
  function convertToPug(section) {
    ...
    function splitOptions(markup) {
      const lastNewline = markup.lastIndexOf('\n');
      if (lastNewline != -1 && markup[lastNewline + 1] == '{') {
        const pugMarkup = markup.substring(0, lastNewline);
        const options = markup.substring(lastNewline, markup.length);
        try {
          return [pugMarkup, JSON.parse(options)];
        } catch (e) {
          console.error(e);
          console.error("styleguide.js: can't parse options " + option);
          return [pugMarkup, {}];
        }
      } else {
        return [markup, {}];
    ...
    // pug options for the section must be on the last markup line
    let splittedOptions  = splitOptions(section.markup);
    const markup = splittedOptions[0];
    const options = splittedOptions[1];

    // use options to explicitly mark pug code
    if (Object.keys(options).length > 0) {
      section.pugFunction = pug.compile(markup);
      section.pugOptions = options;
