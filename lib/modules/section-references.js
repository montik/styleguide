...
function findeAndReplace(section, property, index) {
  var sectionContentVariable = /(<sg\-insert(?:\s+class="(.*)")>)([0-9\.]*)(.*)<\/sg\-insert>/,
      result,
  ...
  function replacer (modifier) {
	...
	// replace adding classes
	function replace(string, placeholder, markup, classes) {
		return "";
	}
  ...
  while ((result = sectionContentVariable.exec(section[property]))) {
    var extraClass = result[2],
			sectionContentIdentifier = result[3],
      sectionModifierIdentifier = result[4],
      referencedSectionContent = '',
      sectionContentReferenceTag = result[1] + sectionContentIdentifier + sectionModifierIdentifier + '</sg-insert>';
