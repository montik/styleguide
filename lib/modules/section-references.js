...
function replaceInsertTags(section, index) {
	...
	// String -> {}
	function parsePugParameters(parameters) {
		try {
			return JSON.parse(parameters);
		} catch(e) {
			console.error(`section-references.js: can not parse pug parameters ${section.reference} ${e}`);
			return {};
		};
	}
	...
	function replaceInsertTagsI(markup) {
		...
		dom.window.document.body.querySelectorAll('sg-insert').forEach(insertTag =>{
			...
			const pugParameters = parsePugParameters(insertTag.getAttribute('parameters'));

			if (!referencedSection || !referencedSection.markup ) {
//				console.error(`section-references.js: section ${insertTag.textContent} referenced from ${section.reference} has no markup`);
				return '';
			}
