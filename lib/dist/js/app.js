...
var deps = ['ui.router', 'ngAnimate', 'colorpicker.module', 'hljs', 'LocalStorageModule', 'oc.lazyLoad', 'ngProgress', 'rt.debounce', 'duScroll'];

if (window._styleguideConfig && window._styleguideConfig.additionalNgDependencies && window._styleguideConfig.additionalNgDependencies.length && window._styleguideConfig.additionalNgDependencies.length > 0) {
  deps = deps.concat(window._styleguideConfig.additionalNgDependencies);
  console.info('Merging dependencies: ' + deps);
}

var a = angular.module('sgApp', deps).value('duScrollOffset', 30).config(["$compileProvider", "$stateProvider", "$urlRouterProvider", "$locationProvider", "localStorageServiceProvider", "$ocLazyLoadProvider", function ($compileProvider, $stateProvider, $urlRouterProvider, $locationProvider, localStorageServiceProvider, $ocLazyLoadProvider) {
  $compileProvider.debugInfoEnabled(false); // disable comments in HTML - e.g. ng-if
...
}).filter('addSectionWrapper', function () {
  return function (code, wrapper) {
    if (wrapper) {
      code = wrapper.replace(/<sg\-wrapper\-content\/>/, code);
    }
    return code;
  };
})
...
angular.module('sgApp').controller('AppCtrl', ["$scope", "ngProgressFactory", "$http", "$location", function ($scope, ngProgressFactory, $http, $location) {
  ...
  if ($location.search().theme == undefined) {
    ...
    $scope.themes = "";
    $http.get('/typo3conf/ext/providerece/Resources/Public/styleguideData/themes.json').then(function (data) {
      $scope.themes = data.data.themes.map(function (theme) {
    ...
    $http.get('/404').then(function (data) {
      var parser = new DOMParser();
      var doc = parser.parseFromString(data.data, 'text/html');
      var hostCss = doc.querySelector("[href*=merged]").getAttribute("href");
      ...
      return console.log("Styleguide main.js error: " + error);
    });
  } else {
    $scope.updateCurrentCss({
      "center": "Host",
      "url": "/typo3conf/ext/providerece/Resources/Public/dev/css/" + $location.search().theme + ".css",
      "linie": "Host"
    });
  }
  ...
  $scope.$on('styles changed', function () {
    ...
    links.forEach(function (link) {
      if ((typeof link === 'undefined' ? 'undefined' : _typeof(link)) === 'object' && link.getAttribute('type') === 'text/css' && link.getAttribute('data-noreload') === null) {
...
angular.module('sgApp').controller('ElementCtrl', ["$scope", "$rootScope", "$stateParams", "$state", "Styleguide", "Variables", "$filter", "$location", function ($scope, $rootScope, $stateParams, $state, Styleguide, Variables, $filter, $location) {
...
angular.module('sgApp').controller('MainCtrl', ["$http", "$scope", "$location", "$state", "Styleguide", "Variables", "localStorageService", "Socket", "Order", function ($http, $scope, $location, $state, Styleguide, Variables, localStorageService, Socket, Order) {
...
angular.module('sgApp').controller('SectionsCtrl', ["$scope", "$stateParams", "$location", "$state", "$rootScope", "$http", "Styleguide", function ($scope, $stateParams, $location, $state, $rootScope, $http, Styleguide) {
  ...
  $scope.replaceCss = function (css, e) {
    console.log(e);
    console.log(css);
  };
...
angular.module('sgApp').controller('VariablesCtrl', ["$rootScope", "$scope", "$stateParams", "$location", "Styleguide", function ($rootScope, $scope, $stateParams, $location, Styleguide) {
...
angular.module('sgApp').directive('sgDesign', ["Variables", "Styleguide", function (Variables, Styleguide) {
  return {
    replace: true,
    restrict: 'A',
    ...
    link: function link(scope) {
      ...
      scope.$watch('currentReference.section', function () {
        ...
        if (scope.showRelated && relatedVariables.length === 0 && scope.sections.data) {
          ...
          scope.relatedChildVariableNames = scope.sections.data.filter(isSubSection).map(getVariables).reduce(concat, []).filter(unique);
        }
      });
...
angular.module('sgApp').directive('dynamicCompile', ["$compile", "$parse", "$window", function ($compile, $parse, $window) {
  return {
...
angular.module('sgApp').directive('routeCssClass', ["$rootScope", function ($rootScope) {
  return {
    restrict: 'A',
    scope: {},
...
angular.module('sgApp').directive('sgSection', ["$rootScope", "$window", "$timeout", function ($rootScope, $window, $timeout) {
  return {
    replace: true,
    restrict: 'A',
...
angular.module('sgApp').directive('shadowDom', ["Styleguide", "$templateCache", function (Styleguide, $templateCache) {
...
angular.module('sgApp').directive('sgVariable', function () {
  return {
    replace: true,
    restrict: 'A',
...
angular.module('sgApp').service('Order', ["$http", "$rootScope", "Socket", "debounce", function ($http, $rootScope, Socket, debounce) {
...
angular.module('sgApp').service('Socket', ["$rootScope", "$window", function ($rootScope, $window) {
...
angular.module('sgApp').service('Styleguide', ["$http", "$rootScope", "Socket", "debounce", "Order", function ($http, $rootScope, Socket, debounce, Order) {
  ...
  this.orderByJson = function (a, b, jsonOrder) {
    var compare = function compare(a, b, section) {
    ...
    var compareLists = function compareLists(a, b, category) {
      if (a.length == 0) {
        return -1;
      } else if (b.length == 0) {
        return 1;
        ...
        a.shift();
        b.shift();
        return compareLists(a, b, _category);
      } else {
  ...
  this.isSectionInFile = function (section, order) {
    var isReferenceInFile = function isReferenceInFile(reference, category) {
      if (reference.length == 0) {
        return true;
      }
      var x = reference.shift();
  ...
  this.get = function () {
    Promise.all([$http.get('styleguide.json'), $http.get('../styleguideData/order.json')]).then(function (values) {
      var styleguide = values[0].data;
      var order = values[1].data;
      _this.config.data = styleguide.config;
      _this.variables.data = styleguide.variables;
      _this.sections.data = styleguide.sections.filter(function (section) {
        return _this.isSectionInFile(section, order);
      }).sort(function (a, b) {
        return _this.orderByJson(a.reference, b.reference, order);
      });
...
(function () {
  ...
  var Variables = function Variables(Styleguide, $q, $rootScope, Socket) {
