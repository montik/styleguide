...
const a = angular.module('sgApp', deps)
...
angular.module('sgApp')
  .controller('MainCtrl', ["$http", "$scope", "$location", "$state", "Styleguide", "Variables", "localStorageService", "Socket", function($http, $scope, $location, $state, Styleguide, Variables, localStorageService, Socket) {
		...
		.then(
			(data) => $scope.themes = data.data.themes.map(theme => {
				theme.url = $scope.stylesheets + theme.center + '.css'; return theme}
			),
...
angular.module('sgApp')
  .controller('SectionsCtrl', ["$scope", "$stateParams", "$location", "$state", "$rootScope", "$http", "Styleguide", function($scope, $stateParams, $location, $state, $rootScope, $http, Styleguide) {
