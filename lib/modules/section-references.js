...
function replaceInsertTags(section, index) {
	...
	function replaceInsertTagsI(markup) {
		...
		dom.window.document.body.querySelectorAll('sg-insert').forEach(insertTag =>{
			const sectionAndModifier = insertTag.textContent.trim().split('-');
			const referencedSection = index[sectionAndModifier[0]];
			const modifier = parseInt(sectionAndModifier[1] || 1) - 1;
			const extraClass = insertTag.classList.toString();

	    if (!referencedSection || !referencedSection.markup ) {
				console.error("section-references.js: the section " + sectionAndModifier[0] + " has no markup");
				return '';
			}
